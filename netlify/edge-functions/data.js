...
export default async function handler(request) {
  ...
  try {
    ...
    const oiNow  = await safeJson(`https://
fapi.binance.com/fapi/v1/openInterest?symbol=${SYMBOL}`);
    const oiHist = await safeJson(`https://fapi.binance.com/futures/data/openInterestHist?symbol=${SYMBOL}&period=1h&limit=24`);
    if (!oiNow.openInterest || !oiHist[0] || !oiHist[0].sumOpenInterest) throw new Error('OI shape');
  ...
  // Wrap as HTML with <pre id="dashboard-data"> so ChatGPT can scrape
