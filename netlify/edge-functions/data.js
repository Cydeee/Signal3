...
export default async function handler(request) {
  ...
  for (const tf of ['15m','1h','4h','1d']) {
    try {
      ...
      result.dataB[tf] = { pct: 0, note: 'error' }
    }
  }
  ...
  try {
    ...
    const now = Date.now()
    const windows = { '15m': .25, '1h': 1, '4h': 4, '24h': 24 }
  ...
  try {
    ...
    if (!oiNow.openInterest || !oiHist[0] || !oiHist[0].sumOpenInterest) {
  ...
  try {
    ...
    const up = cg.sentiment_votes_up_percentage
             != null ? cg.sentiment_votes_up_percentage
  ...
  try {
    ...
    const g  = gv.data
    if (!g || !g.total_market_cap || !g.total_market_cap.usd) {
