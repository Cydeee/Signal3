  ...
  try {
    ...
    try {
      ...
      for (const [lbl,hrs] of Object.entries(windows)) {
        const cut = now - hrs*3600000, bullArr = 0, bearArr = 0;
        let bull=0, bear=0;
        for (const k of rows) {
          if (+k[0] < cut) continue;
      ...
      for (const lbl of ['15m','1h','4h']) {
        ...
        let tag;
        if      (r>2)   tag='very high';
        else if (r>1.2) tag='high';
        else if (r<0.5) tag='low';
        else            tag='normal';
        result.dataC.relative[lbl] = tag;
      }
    } catch (e) {
    ...
    try {
      ...
      if (!gv.data || !gv.data.total_market_cap?.usd) throw new Error('global missing');
      const g = gv.data;
      result.dataF = {
    ...
    // Wrap in HTML for ChatGPT scraping
...
<pre id="dashboard-data">${JSON.stringify(result)}</pre>
    ...
    console.error("Edge function fatal error:", fatal);
    return new Response(
      `<!DOCTYPE html><body><h1>Service unavailable</h1></body></html>`,
