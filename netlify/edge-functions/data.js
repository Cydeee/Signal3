...
async function buildDashboardData() {
  ...
  const std=(a,p)=>{const sl=a.slice(-p), m=sma(sl,p); return Math.sqrt(sl.reduce((t,x)=>t+(x-m)**2,0)/p);};
  ...
  const rsi=(a,p)=>{if(a.length<p+1)return 0;let g=0,l=0;for(let i=1;i<=p;i++){const d=a[i]-a[i-1]; d>=0?g+=d:l-=d;}let ag=g/p, al=l/p;for(let i=p+1;i<a.length;i++){const d=a[i]-a[i-1]; ag=(ag*(p-1)+Math.max(d,0))/p; al=(al*(p-1)+Math.max(-d,0))/p;} return al? 100 - 100/(1+ag/al) : 100; };
  const atr=(h,l,c,p)=>{if(h.length<p+1) return 0; const trs=[]; for(let i=1;i<h.length;i++){ trs.push(Math.max(h[i]-l[i], Math.abs(h[i]-c[i-1]), Math.abs(l[i]-c[i-1])));} return sma(trs.slice(-p), p);};
  ...
  for (const tf of ["15m","1h","4h","1d"]) {
    ...
    } catch(e) { result.errors.push(`B[${tf}]: ${e.message}`); }
  }
  ...
  } catch(e) { result.errors.push("C: "+e.message); }
